...
public class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    public DbSet<ChapterPlotPoint> ChaptersPlotPoints { get; set; }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        ConfigureChapterPlotPoint(modelBuilder);
        ...
        ConfigureJoin<ChapterPlotPoint>(modelBuilder, x => new { x.ChapterId, x.PlotPointId });
    ...
    private static void ConfigureDate(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Event>()
            .HasOne(e => e.StartDate)
            .WithOne() // or .WithMany() if Date had multiple Events — but it doesn't need this.
            .HasForeignKey<Event>("StartDateId")
            .IsRequired(false);

        modelBuilder.Entity<Event>()
            .HasOne(e => e.EndDate)
            .WithOne()
            .HasForeignKey<Event>("EndDateId")
            .IsRequired(false);

        modelBuilder.Entity<Era>()
            .HasOne(e => e.StartDate)
            .WithOne() // or .WithMany() if Date had multiple Eras — but it doesn't need this.
            .HasForeignKey<Era>("StartDateId")
            .IsRequired(false);
        modelBuilder.Entity<Era>()
            .HasOne(e => e.EndDate)
            .WithOne() // or .WithMany() if Date had multiple Eras — but it doesn't need this.
            .HasForeignKey<Era>("EndDateId")
            .IsRequired(false);

        modelBuilder.Entity<Chapter>()
            .HasOne(c => c.StartDate)
            .WithOne() // or .WithMany() if Date had multiple Chapters — but it doesn't need this.
            .HasForeignKey<Chapter>("StartDateId")
            .IsRequired(false);

        modelBuilder.Entity<Chapter>()
            .HasOne(c => c.EndDate)
            .WithOne() // or .WithMany() if Date had multiple Chapters — but it doesn't need this.
            .HasForeignKey<Chapter>("EndDateId")
            .IsRequired(false);

        modelBuilder.Entity<PlotPoint>()
            .HasOne(c => c.StartDate)
            .WithOne() // or .WithMany() if Date had multiple PlotPoints — but it doesn't need this.
            .HasForeignKey<PlotPoint>("StartDateId")
            .IsRequired(false);

        modelBuilder.Entity<PlotPoint>()
            .HasOne(c => c.EndDate)
            .WithOne() // or .WithMany() if Date had multiple PlotPoints — but it doesn't need this.
            .HasForeignKey<PlotPoint>("EndDateId")
            .IsRequired(false);

        modelBuilder.Entity<Character>()
            .HasOne(c => c.BirthDate)
            .WithOne() // or .WithMany() if Date had multiple Characters — but it doesn't need this.
            .HasForeignKey<Character>("BirthDateId")
            .IsRequired(false);
    ...
    private static void ConfigureChapterPlotPoint(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<ChapterPlotPoint>()
            .HasKey(cpp => new { cpp.ChapterId, cpp.PlotPointId });
        modelBuilder.Entity<ChapterPlotPoint>()
            .HasOne(cpp => cpp.Chapter)
            .WithMany(c => c.ChapterPlotPoints)
            .HasForeignKey(cpp => cpp.ChapterId)
            .OnDelete(DeleteBehavior.Restrict);
        modelBuilder.Entity<ChapterPlotPoint>()
            .HasOne(cpp => cpp.PlotPoint)
            .WithMany(p => p.PlotPointChapters)
            .HasForeignKey(cpp => cpp.PlotPointId)
            .OnDelete(DeleteBehavior.Restrict);
    }
