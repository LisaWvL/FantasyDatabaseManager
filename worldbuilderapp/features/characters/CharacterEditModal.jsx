...
import { fetchCharacters, deleteCharacter, updateCharacter } from '../api/CharacterApi';
import { fetchFactions, fetchLocations, fetchLanguages } from '../api/DropdownApi';
import { fetchChapters } from '../api/ChapterApi.ts';
...
export default function CharacterPage() {
    ...
    const loadData = async () => {
        try {
            const [characterData, chapterData, factionsData, locationsData, languagesData] =
                await Promise.all([
                    fetchCharacters(),
                    fetchChapters(),
                    fetchFactions(),
    ...
    const relevantChapters = chapters.filter(
        (c) => c.chapterTitle || c.chapterText || c.chapterSummary || c.toDo
    );
            ...
            {relevantChapters.map((chap) => (
                <section key={chap.id} className="chapter-section">
                    <h2 className="chapter-title">
                        Book {chap.bookNumber} • Act {chap.actNumber} • Chapter {chap.chapterNumber}:{' '}
                        {chap.chapterTitle}
                    </h2>
                    <div className="chapter-card-list">
                        {(charactersByChapter[chap.id] || []).map((char) => (
                            <React.Fragment key={char.id}>
                                <CharacterCard
                                    chapters={(charactersByChapter[chap.id] || []).filter(
                                        (c) => c.name === char.name
                                    )}
