...
//TODO this page shows my story on a timeline axis, with the main story being driven by Plotpoints and the writing progress by snapshots that link to versions of entities
...
//there is a way to switch between showing snapshot or plotpoints or both
...
export default function TimelineStoryView() {
    const [viewMode, setViewMode] = useState("both"); // Options: 'both', 'plotpoints', 'snapshots'
    ...
    return (
        <div className="timeline-story-view">
            ...
            <div className="timeline-controls">
                <label>Show:</label>
                <button onClick={() => handleViewToggle("both")}>PlotPoints + Snapshots</button>
                <button onClick={() => handleViewToggle("plotpoints")}>PlotPoints Only</button>
                <button onClick={() => handleViewToggle("snapshots")}>Snapshots Only</button>
            </div>
