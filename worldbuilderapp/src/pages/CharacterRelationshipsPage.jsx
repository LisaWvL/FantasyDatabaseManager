...
import { useRowEditKeyboard } from '../hooks/useRowEditKeyboard';
import '../styles/CharacterRelationshipPage.css';
...
export default function CharacterRelationshipPage() {
    ...
    const filtered = relationships.filter(r => {
        ...
        return (
            (!characterId || r.character1Id === parseInt(characterId) || r.character2Id === parseInt(characterId)) &&
                ...
                <tbody>
                    {filtered.map(rel => (
                        ...
                        >
                            <td>
                                {editingRowId === rel.id ? (
                                    <select name="character1Id" value={editedData.character1Id || ''} onChange={handleChange}>
                                        {characters.map(c => (
                                            <option key={c.id} value={c.id}>{c.name}</option>
                                        ))}
                                    </select>
                            ...
                            <td>
                                {editingRowId === rel.id ? (
                                    <select name="character2Id" value={editedData.character2Id || ''} onChange={handleChange}>
                                        {characters.map(c => (
                                            <option key={c.id} value={c.id}>{c.name}</option>
                                        ))}
                                    </select>
                            ...
                            <td>
                                {editingRowId === rel.id ? (
                                    <input name="relationshipType" value={editedData.relationshipType || ''} onChange={handleChange} />
                                ) : (
                            ...
                            <td>
                                {editingRowId === rel.id ? (
                                    <select name="chapterId" value={editedData.chapterId || ''} onChange={handleChange}>
                                        {chapters.map(s => (
                                            <option key={s.id} value={s.id}>{s.chapterName}</option>
                                        ))}
                                    </select>
