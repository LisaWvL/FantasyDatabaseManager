...
export const fetchPlotPointsForChapter = async (chapterId) => {
    try {
        ...
        console.error(`❌ Failed to fetch plot points for chapter ${chapterId}:`, error.response?.data || error.message);
        return [];
    }
};
...
export const createPlotPointWithLinkedEntities = async (payload) => {
    try {
        ...
        console.error('❌ Failed to create plot point with entities:', error.response?.data || error.message);
        throw error;
    }
};
...
export const linkPlotPointToEntity = async (plotPointId, entityType, entityId) => {
    try {
        ...
        console.error(`❌ Failed to link ${entityType} (${entityId}) to plot point ${plotPointId}:`, error.response?.data || error.message);
        throw error;
    }
};
...
export const unlinkPlotPointFromEntity = async (plotPointId, entityType, entityId) => {
    try {
        ...
        console.error(`❌ Failed to unlink ${entityType} (${entityId}) from plot point ${plotPointId}:`, error.response?.data || error.message);
        throw error;
    }
};
