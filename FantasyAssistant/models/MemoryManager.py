...
# models/MemoryManager.py

from models.TokenUtils import estimate_tokens

MAX_CONTEXT_TOKENS = 2048
RESERVED_FOR_SYSTEM = 300

# Filter and truncate history to fit within token limit

def get_turns_for_context(prior_turns, system_prompt):
    context = []
    total_tokens = estimate_tokens(system_prompt) + RESERVED_FOR_SYSTEM

    for turn in reversed(prior_turns):
        user_text = turn.get("prompt", "")
        assistant_text = turn.get("response", "")

        user_tokens = estimate_tokens(user_text)
        assistant_tokens = estimate_tokens(assistant_text)

        if total_tokens + user_tokens + assistant_tokens > MAX_CONTEXT_TOKENS:
            break

        context.insert(0, {"role": "assistant", "content": assistant_text})
        context.insert(0, {"role": "user", "content": user_text})
