...
import { fetchPlotPointsForChapter } from '../plotpoints/PlotPointApi';
...
export default function WritingAssistantPage() {
  ...
  useEffect(() => {
    fetchChapters().then(async (all) => {
      ...
      const filtered = all.filter(
        (s) => s.chapterTitle || s.chapterText || s.summary || s.povCharacterName || s.toDo
      );
  ...
  const handleSave = (updatedChapter) => {
    setChapterHeaders((prev) => prev.map((s) => (s.id === updatedChapter.id ? updatedChapter : s)));
  };
